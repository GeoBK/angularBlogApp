...
export class CreateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ar.params.subscribe(params=>{
      if(params['id']){
        this._articleService.getArticle(params['id']).subscribe(res=>{
          this.article=res;
          this.articleForm=this.fb.group({
            'title':[this.article['title'],Validators.compose([Validators.required,Validators.minLength(10),Validators.maxLength(45)])],
            'content':[this.article['content'],Validators.compose([Validators.required,Validators.minLength(10),Validators.maxLength(45)])]
          });
        });
        ...
        this.articleForm=this.fb.group({
          ...
          'content':[null,Validators.compose([Validators.required,Validators.minLength(10),Validators.maxLength(45)])]
        });
      }
    })
  ...
  addArticle(articleId,article:Article){
    if(articleId!==undefined){
      this._articleService.updateArticle(article,articleId._id).subscribe(newArticle=>{
